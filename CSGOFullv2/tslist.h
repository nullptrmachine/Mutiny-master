...
public:
	...
	TSLNodeBase_t *Push(TSLNodeBase_t *pNode)
	{
		...
		for (;;)
		{
			...
			*((uint32_t *)&newHead.value.Depth) = *((uint32_t *)&oldHead.value.Depth) + 0x10001;
	...
	TSLNodeBase_t *Pop()
	{
		...
		for (;;)
		{
			...
			*((uint32_t *)&newHead.value.Depth) = *((uint32_t *)&oldHead.value.Depth) - 1;
	...
	TSLNodeBase_t *Detach()
	{
		...
		do
		{
			...
			*((uint32_t *)&newHead.value.Depth) = *((uint32_t *)&oldHead.value.Depth) & 0xffff0000;
