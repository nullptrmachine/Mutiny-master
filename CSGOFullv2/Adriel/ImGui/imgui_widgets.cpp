...
bool ImGui::CloseButton(ImGuiID id, const ImVec2& pos)//, float size)
{
	...
	auto text_size = CalcTextSize(u8"\uf00d");
	ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_CheckMark : ImGuiCol_Text);
	ImVec2 center = bb.GetCenter() + ImVec2(-5.f, -5.f);

	window->DrawList->AddText(center, col, u8"\uf00d");
    return pressed;
}
