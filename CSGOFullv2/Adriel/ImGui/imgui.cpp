...
void ImGui::Render(ImDrawList* OverlayDrawList)
{
    ImGuiContext& g = *GImGui;
    IM_ASSERT(g.Initialized);
	...
	if (OverlayDrawList && !OverlayDrawList->VtxBuffer.empty())
		AddDrawListToDrawData(&g.DrawDataBuilder.Layers[0], OverlayDrawList);
...
inline float ImVec2::Length(void) const 
{ 
	return (float)sqrtf(x*x + y * y); 
}

float ImVec2::Normalise(void)
{
...
ImVec2 ImVec2::Direction(void) const
{
	ImVec2 Temp(*this);
...
float ImVec2::Angle(const ImVec2& xE)
{
	float dot = (*this).Dot(xE);
	float cross = (*this).Cross(xE);
...
ImVec2& ImVec2::Rotate(float angle)
{
	float tx = x;
...
ImVec2& ImVec2::Rotate(const ImVec2& xCentre, float fAngle)
{
	ImVec2 D = *this - xCentre;
	D.Rotate(fAngle);
...
void ImVec2::Clamp(const ImVec2& min, const ImVec2& max)
{
