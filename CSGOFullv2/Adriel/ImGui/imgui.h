...
struct ImVec2
{
	...
	ImVec2& operator=(const ImVec2 &vOther)
	{
		x = vOther.x;
		y = vOther.y;
		return *this;
	}
	int operator==(const ImVec2 &vOther) const
	{
		return x == vOther.x && y == vOther.y;
	}

	ImVec2 &operator /=(const float Scalar) 
	{
		x /= Scalar; 
		y /= Scalar;		
		return *this; 
	...
	//ImVec2 &operator +=(const ImVec2 &Other) 
	//{ 
	//	x += Other.x;	
	//	y += Other.y;	
	//	return *this; 
	...
	//inline float operator ^ (const ImVec2 &V)	const 
	//{ 
	//	return (x * V.y) - (y * V.x); 
	//} // cross product

	inline float Cross(const ImVec2&V) const 
	{ 
		return (x * V.y) - (y * V.x); 
	}
	...
	inline float Dot(const ImVec2&V) const 
	{ 
		return (x*V.x) + (y*V.y); 
	}

	//ImVec2 operator * (float  s)			const 
	//{
	...
	//const ImVec2 operator - (const ImVec2 &V) const 
	//{ 
	//	return ImVec2(x - V.x, y - V.y); 
	//}

	friend ImVec2 operator * (float k, const ImVec2& V) 
	{ 
		return ImVec2(V.x*k, V.y*k); 
	}


	ImVec2 operator -(void) const 
	{ 
		return ImVec2(-x, -y); 
	}
	...
	ImVec2 Direction(void) const;

	float Angle(const ImVec2& xE);

	ImVec2& Rotate(float angle);

	ImVec2& Rotate(const ImVec2& xCentre, float fAngle);

	void Clamp(const ImVec2& min, const ImVec2& max);
};
...
namespace ImGui
{
    ...
    IMGUI_API void          Render(ImDrawList* OverlayDrawList);        // ends the Dear ImGui frame, finalize the draw data. You can get call GetDrawData() to obtain it and run your rendering function. (Obsolete: this used to call io.RenderDrawListsFn(). Nowadays, we allow and prefer calling your render function yourself.)
...
struct ImFontAtlas
{
	...
	IMGUI_API const ImWchar* GetOnlyGlyphBasicRanges();
	IMGUI_API const ImWchar* GetOnlyGlyphRangesCyrillic();
