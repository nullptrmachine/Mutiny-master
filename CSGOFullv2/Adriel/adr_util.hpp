...
namespace adr_util
{
	...
	std::string get_weapon_name(ItemDefinitionIndex index);
	bool is_knife(ItemDefinitionIndex index);
	bool is_glove(ItemDefinitionIndex index);
	bool is_rifle(ItemDefinitionIndex index);
	bool is_smg(ItemDefinitionIndex index);
	bool is_shotgun(ItemDefinitionIndex index);
	bool is_sniper(ItemDefinitionIndex index);
	bool is_pistol(ItemDefinitionIndex index);
	bool is_heavy(ItemDefinitionIndex index);
	bool is_utility(ItemDefinitionIndex index);

	std::string sanitize_name(char* name);

	struct weapon_name_t
	{
		weapon_name_t(short i_definition_index, const char* ch_name)
		{
			definition_index = i_definition_index;
			name = ch_name;
		}
	...
	//extern std::vector< weapon_name_t > aim_weapon_names;
}