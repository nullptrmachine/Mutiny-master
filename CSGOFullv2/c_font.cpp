...
c_font::c_font() : height(0), flags(0), quality(0), weight(0), heighest_char(0), dev(nullptr), texture(nullptr),
vertex_buffer(nullptr), text_width(0), text_height(0), text_scale(0), spacing(0), saved(nullptr),
draw(nullptr), line_height(0) {}

c_font::c_font(std::string font, const uint32_t height, const uint32_t weight, const uint32_t highest_char, const uint32_t flags, const uint32_t quality) : font(
	std::
	move(font)), height(height),
	flags(flags), quality(quality), weight(weight),
