...
#define AssertValidWritePtr(a,b)
#define AssertValidReadPtr(a,b)
#define AssertValidStringPtr(a)
...
//			maxoutputbytes - 
//-----------------------------------------------------------------------------
void V_hextobinary(char const *in, int numchars, char *out, int maxoutputbytes)
{
	int len = V_strlen(in);
...
void V_AppendSlash(char *pStr, int strSize)
{
	...
	{
		if (len + 1 >= strSize)
			exit(EXIT_SUCCESS);//Error("V_AppendSlash: ran out of space on %s.", pStr);
...
void V_MakeAbsolutePath(char *pOut, int outLen, const char *pPath, const char *pStartingDir)
{
	...
	else
	{
		...
		else
		{
			if (!_getcwd(pOut, outLen))
				exit(EXIT_SUCCESS);//Error("V_MakeAbsolutePath: _getcwd failed.");
	...
	if (!V_RemoveDotSlashes(pOut))
		exit(EXIT_SUCCESS);//Error("V_MakeAbsolutePath: tried to \"..\" past the root.");
