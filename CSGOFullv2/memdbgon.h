...
// Dylan note: this is a cheat, don't use debug mode for valve's memory class or we will crash! the game is not compiled in debug mode
...
#define free(p)				GetValveAllocator()->Free( p )
#define _msize(p)			GetValveAllocator()->GetSize( p )
...
#define malloc(s)				GetValveAllocator()->Alloc( s, __FILE__, __LINE__)
#define realloc(p, s)			GetValveAllocator()->Realloc( p, s, __FILE__, __LINE__ )
...
inline char *MemAlloc_StrDup(const char *pString, const char *pFileName, unsigned nLine)
{
	...
	if ((pMemory = (char *)GetValveAllocator()->Alloc(len, pFileName, nLine)) != NULL)
	{
...
inline wchar_t *MemAlloc_WcStrDup(const wchar_t *pString, const char *pFileName, unsigned nLine)
{
	...
	if ((pMemory = (wchar_t *)GetValveAllocator()->Alloc(len * sizeof(wchar_t), pFileName, nLine)) != NULL)
	{
...
#define malloc(s)				GetValveAllocator()->Alloc( s )
#define realloc(p, s)			GetValveAllocator()->Realloc( p, s )
...
inline char *MemAlloc_StrDup(const char *pString)
{
	...
	size_t len = strlen(pString) + 1;
	if ((pMemory = (char *)GetValveAllocator()->Alloc(len)) != NULL)
	{
		return strcpy(pMemory, pString);
	}
...
inline wchar_t *MemAlloc_WcStrDup(const wchar_t *pString)
{
	...
	size_t len = (wcslen(pString) + 1);
	if ((pMemory = (wchar_t *)GetValveAllocator()->Alloc(len * sizeof(wchar_t))) != NULL)
	{
		return wcscpy(pMemory, pString);
	}
