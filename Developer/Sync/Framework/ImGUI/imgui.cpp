...
void ImGuiIO::AddInputCharactersUTF8(const char* utf8_chars)
{
	...
	const int wchars_buf_len = sizeof(InputCharacters) / sizeof(ImWchar);
...
{
	ImVector<Pair>::iterator it = LowerBound(const_cast<ImVector<Pair>&>(Data), key);
...
void ImGui::NewFrame()
{
	...
	if (!g.Initialized)
	{
		...
		g.LogClipboard = (ImGuiTextBuffer*)MemAlloc(sizeof(ImGuiTextBuffer));
	...
	SetNextWindowSize(ImVec2(400, 400), ImGuiSetCond_FirstUseEver);
	Begin("Debug");
}
...
bool ImGui::BeginChild(const char* str_id, const ImVec2& size_arg, bool border, ImGuiWindowFlags extra_flags)
{
	...
	ImGuiWindow* window = GetCurrentWindow();
	return BeginChildEx(str_id, window->GetID(str_id), Check, border, extra_flags);
}
...
bool ImGui::BeginChild(ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags extra_flags)
{
	...
	return BeginChildEx(nullptr, id, Check, border, extra_flags);
}
...
{
	return Begin(name, p_open, ImVec2(0.f, 0.f), -1.0f, flags);
}
...
bool ImGui::Begin(const char* name, bool* p_open, const ImVec2& size_on_first_use, float bg_alpha, ImGuiWindowFlags flags)
{
	...
	if (first_begin_of_the_frame)
	{
		...
		if (!(flags & ImGuiWindowFlags_NoTitleBar))
		{
			if (p_open != nullptr && !(flags & ImGuiWindowFlags_NoClose))
			{
...
//   "Gold: %.0f"   Gold: 1
bool ImGui::SliderFloatTop(const char* label, float* v, float v_min, float v_max, const char* display_format, float power)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	const ImVec2 label_size = CalcTextSize(label, nullptr, true);
	const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, label_size.y), window->DC.CursorPos + ImVec2(w * 1.4, label_size.y + style.FramePadding.y * 2.0f) + ImVec2(0, label_size.y));
	const ImRect total_bb(frame_bb.Min - ImVec2(0, label_size.y), frame_bb.Max);
	...
	if (label_size.x > 0.0f)
		RenderText(ImVec2(frame_bb.Min.x, frame_bb.Min.y - label_size.y), label);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
	...
	if (*v)
	{
		...
		//ndow->DrawList->AddText(22.f,check_bb.Min + ImVec2(pad, pad), GetColorU32(ImGuiCol_CheckMark), "X");
	}
...
{
	static const float HUE_PICKER_WIDTH = 10.0f;
	...
	if ((IsItemHovered() || IsItemActive()) && GetIO().MouseDown[0])
	{
		ImVec2 mouse_pos_in_canvas = ImVec2(
			GetIO().MousePos.x - picker_pos.x, GetIO().MousePos.y - picker_pos.y);
	...
	col[1] = color.Value.y;
	col[2] = color.Value.z;
	return value_changed | ColorEdit3(label, col);
}
...
{
	if (Begin("ImGui Metrics", p_open))
	{
		Text("ImGui %s", GetVersion());
		Text("Application average %.3f ms/frame (%.1f FPS)", 1000.0f / GetIO().Framerate, GetIO().Framerate);
		Text("%d vertices, %d indices (%d triangles)", GetIO().MetricsRenderVertices, GetIO().MetricsRenderIndices, GetIO().MetricsRenderIndices / 3);
		Text("%d allocations", GetIO().MetricsAllocs);
		static bool show_clip_rects = true;
		Checkbox("Show clipping rectangles when hovering a ImDrawCmd", &show_clip_rects);
		Separator();
		...
		struct Funcs
		{
			static void NodeDrawList(ImDrawList* draw_list, const char* label)
			{
				...
				{
					SameLine();
					TextColored(ImColor(255, 100, 100), "CURRENTLY APPENDING"); // Can't display stats for active draw list! (we don't have the data double-buffered)
				...
				for (const ImDrawCmd* pcmd = draw_list->CmdBuffer.begin(); pcmd < draw_list->CmdBuffer.end(); elem_offset += pcmd->ElemCount , pcmd++)
				{
					...
					bool pcmd_node_open = TreeNode((void*)(pcmd - draw_list->CmdBuffer.begin()), "Draw %-4d %s vtx, tex = %p, clip_rect = (%.0f,%.0f)..(%.0f,%.0f)", pcmd->ElemCount, draw_list->IdxBuffer.Size > 0 ? "indexed" : "non-indexed", pcmd->TextureId, pcmd->ClipRect.x, pcmd->ClipRect.y, pcmd->ClipRect.z, pcmd->ClipRect.w);
			...
			static void NodeWindow(ImGuiWindow* window, const char* label)
			{
				...
				BulletText("Pos: (%.1f,%.1f)", window->Pos.x, window->Pos.y);
				...
				BulletText("Storage: %d bytes", window->StateStorage.Data.Size * (int)sizeof(ImGuiStorage::Pair));
				TreePop();
			}
		};
		...
		if (TreeNode("DrawList", "Active DrawLists (%d)", g.RenderDrawLists[0].Size))
		{
			for (int i = 0; i < g.RenderDrawLists[0].Size; i++)
				Funcs::NodeDrawList(g.RenderDrawLists[0][i], "DrawList");
			TreePop();
		}
		if (TreeNode("Popups", "Open Popups Stack (%d)", g.OpenPopupStack.Size))
		{
		...
		{
			Text("FocusedWindow: '%s'", g.FocusedWindow ? g.FocusedWindow->Name : "NULL");
			Text("HoveredWindow: '%s'", g.HoveredWindow ? g.HoveredWindow->Name : "NULL");
			Text("HoveredRootWindow: '%s'", g.HoveredRootWindow ? g.HoveredRootWindow->Name : "NULL");
			Text("HoveredID: 0x%08X/0x%08X", g.HoveredId, g.HoveredIdPreviousFrame); // Data is "in-flight" so depending on when the Metrics window is called we may see current frame information or not
