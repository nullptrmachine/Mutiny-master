...
namespace ImGui
{
	...
	IMGUI_API void ShowStyleEditor(ImGuiStyle* ref = nullptr); // style editor block. you can pass in a reference ImGuiStyle structure to compare to, revert to and save to (else it uses the default style)
	IMGUI_API void ShowTestWindow(bool* p_open = nullptr); // test window demonstrating ImGui features
	IMGUI_API void ShowMetricsWindow(bool* p_open = nullptr); // metrics window for debugging ImGui (browse draw commands, individual vertices, window list, etc.)
	...
	IMGUI_API void TextUnformatted(const char* text, const char* text_end = nullptr); // doesn't require null terminated string if 'text_end' is specified. no copy done to any bounded stack buffer, recommended for long chunks of text
	...
	IMGUI_API bool SliderFloatTop(const char* label, float* v, float v_min, float v_max, const char* display_format = "%.3f", float power = 1.0f); // adjust display_format to decorate the value with a prefix or a suffix. Use power!=1.0 for logarithmic sliders
	...
	static inline bool GetWindowCollapsed() { return IsWindowCollapsed(); } // OBSOLETE 1.39+
...
struct ImDrawList
{
	...
	IMGUI_API void AddText(float font_size, const ImVec2& pos, ImU32 col, const char* text_begin, const char* text_end = nullptr);
	...
	void PathLineToMergeDuplicate(const ImVec2& pos) { if (_Path.Size == 0 || memcmp(&_Path[_Path.Size - 1], &pos, 8) != 0) _Path.push_back(pos); }
